        ...
        if (strpos($bimestre, "recbimestre") !== false) {
            ...
            if(mysqli_num_rows($result_nota) > 0){
                ...
                if(mysqli_query($conn, $sql_update)){
                    header('Location: detalhes.php?id='. $matricula .'&turma='. $turma .'&sucess=Nota alterada com sucesso!');
                    exit();
                }
                else{
                    header('Location: detalhes.php?id='. $matricula .'&turma='. $turma .'&error=Erro ao alterar nota!');
                    exit();
                }
            }
            else{
